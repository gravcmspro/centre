 ...
 * @author GravCMS Pro
 */
class BreadcrumbShortcode extends BaseShortcode
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $breadcrumbsClassFile = __DIR__.'/../../../breadcrumbs/classes/breadcrumbs.php';
        if (!file($breadcrumbsClassFile)) {
            return '';
        }

        require_once $breadcrumbsClassFile;

        $output = $this->grav['twig']->processTemplate($this->template(), [
            'id' => $shortcode->getParameter('id'),
            'name' => $shortcode->getParameter('name'),
            'breadcrumbs' => new \Grav\Plugin\Breadcrumbs($this->config->get('plugins.breadcrumbs')),
        ]);
